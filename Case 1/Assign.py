...
#     for j in range(1,Dpoint):
#         for m in range(1,MaNum):
#               ma.addConstr(quicksum(MaSu[r,i,j,m] for i in np.arange(1,Spoint)) == MaDe[r,j,m])
#               for i in range(1,Spoint):
#                   ma.addConstr(MaStore[i,m] >= MaSu[r,i,j,m])
# for r in range(1,Requests):
...
#             for m in range(1,MaNum):
#                 for k in range(1,TCpoint):
#                     ma.addConstr(2 - ReSe[r,k] - MaSu[r,i,j,m] >= 1 - SuDe[r,i,j,m,k])
...
# ma.Params.OutputFlag = 1 #是否输出求解过程,1为输出，0为不输出
# ma.optimize()
...
#             if ReSe[r,k].x == 1:
#                 Listk.append(r)
#         List.append(Listk)  #将任务编号分配给不用塔吊
#     MaxLen = max(len(x) for x in List)
#     NewList = list(map(lambda l:l + [Requests]*(MaxLen - len(l)), List)) #补全任务数量，转化为array，虚拟任务用物料种类m=0代替
#     AssignResult = RequestDetail[np.array(NewList) - 1] #将总物料运输任务分配给各塔吊
# def Optimize():
#     ma.Params.OutputFlag = 1 #是否输出求解过程,1为输出，0为不输出
#     ma.optimize()
#     if ma.status == GRB.status.OPTIMAL:

...
# List = [[4,6,11,12], [1,7,8,15], [5,9,10,16], [2,3,13,14]]
...
################################# Get the Fitness of each Individual #################################
    def get_fitness(self, supply_point, demand_point, deliver_time, startpoint):
        ...
        total_time = np.sum(Single_time + 2 * total_wait, axis=1) #总时间
...
################################# Evolve Operator for Population #################################    
    def evolve(self, fitness, supply_chain, request, material_store, dis_st):
        ...
        for z in range(self.pop_size): #挑选的父代A
            #Crossover 
            child_popA, child_popB, child_supA, child_supB = self.crossover(z, best_individual, pop, fitness, supply_chain, mean_, max_)
            #sequence mutate
...
################################ Starts #################################
    ...
    for generation in range(N_GENERATIONS):
        ...
        Fitness, OperationTime, WaitingTime = ga.get_fitness(SupplySequence, DemandSequence, DeliverTime, InitLocation)
...
################################# Plot Result #################################    
    plt.figure()
    plt.plot(np.arange(N_GENERATIONS)+1, BF, label = 'max',linewidth = 1)
    plt.plot(np.arange(N_GENERATIONS)+1, MF, label = 'mean', linestyle='--',linewidth = 1)
    plt.xlim(0, N_GENERATIONS)
    plt.xlabel('Iterations')
    plt.ylabel('Total transport cost')
